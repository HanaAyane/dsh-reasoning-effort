...
第一次打开插件时，你会在 DSH 输入框下方看到新的模型入口。点击后，弹层上方是模型声明全部思考档位的滑块（`off` / `minimal` / `low` / `medium` / `high` / `xhigh` / `max` 的子集），下方仍然是熟悉的模型选择入口。插件默认启用，并与 DSH 的 `/model` 命令保持同步。
...
滑块不再写死三档。它读取当前模型公开的 `reasoning.efforts`，有几个档就渲染几档，松手吸附到最近的一档。模型未公开至少两档、部署关闭 thinking，或只支持 `off` 时，滑块会自动隐藏。

各档位含义由模型/网关自行决定；`dsh-model-capabilities` 可以在设置里为自定义路由补齐这些声明。
...
3. 当前模型公开至少两个思考档位，且部署没有关闭 thinking。
