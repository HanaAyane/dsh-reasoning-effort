...
const rawSrc = fs.readFileSync('C:/Users/15528/.dsh/attachments/v1/objects/15/15d1777bcda98c5bdbb86f8a47bbe0912b034da95781a24bee11e73aa0a86ef1');
const srcImg = decodePNG(rawSrc);

console.log('GLM Raw:', srcImg.width, 'x', srcImg.height);

// 1. Precise 2D Connected Component Detection & Isolation
const coreLabels = new Int32Array(srcImg.width * srcImg.height).fill(-1);
const rawComps = [];

for (let y = 0; y < srcImg.height; y++) {
  for (let x = 0; x < srcImg.width; x++) {
    const idx = y * srcImg.width + x;
    if (coreLabels[idx] !== -1 || srcImg.data[idx * 4 + 3] <= 20) continue;

    const q = [idx];
    const compId = rawComps.length;
    coreLabels[idx] = compId;
      ...
      for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
          ...
          if (nx >= 0 && nx < srcImg.width && ny >= 0 && ny < srcImg.height) {
            ...
            if (coreLabels[nidx] === -1 && srcImg.data[nidx * 4 + 3] > 20) {
              coreLabels[nidx] = compId;
              q.push(nidx);
            }
          }
        }
    ...
    if (pixelCount > 1000) {
      rawComps.push({
        rawId: compId,
        pixelCount, minX, maxX, minY, maxY,
...
rawComps.sort((a, b) => a.avgX - b.avgX);
console.log('GLM 8 raw components:');
rawComps.forEach((c, i) => {
  console.log(`Frame ${i}: pixels=${c.pixelCount}, X=[${c.minX}, ${c.maxX}] (w=${c.maxX-c.minX+1}), Y=[${c.minY}, ${c.maxY}] (h=${c.maxY-c.minY+1}), center=(${c.avgX.toFixed(1)}, ${c.avgY.toFixed(1)})`);
});

if (rawComps.length !== 8) {
  throw new Error(`Expected 8 character components, got ${rawComps.length}`);
}

const rawIdToFrameIndex = new Map();
rawComps.forEach((c, i) => rawIdToFrameIndex.set(c.rawId, i));

const frameMask = new Int8Array(srcImg.width * srcImg.height).fill(-1);
for (let i = 0; i < srcImg.width * srcImg.height; i++) {
  if (coreLabels[i] !== -1 && rawIdToFrameIndex.has(coreLabels[i])) {
    frameMask[i] = rawIdToFrameIndex.get(coreLabels[i]);
  }
}

// Expand anti-aliasing edges (0 < alpha <= 20) to nearest character within 3px
const edgeQueue = [];
const dist = new Int16Array(srcImg.width * srcImg.height).fill(999);
for (let i = 0; i < srcImg.width * srcImg.height; i++) {
  if (frameMask[i] >= 0) {
    dist[i] = 0;
    edgeQueue.push(i);
...
let qHead = 0;
while (qHead < edgeQueue.length) {
  const curr = edgeQueue[qHead++];
  const cy = Math.floor(curr / srcImg.width);
  const cx = curr % srcImg.width;
  const curDist = dist[curr];
  const curLabel = frameMask[curr];

  if (curDist >= 3) continue;
  ...
  for (let dy = -1; dy <= 1; dy++) {
    for (let dx = -1; dx <= 1; dx++) {
      ...
      if (nx >= 0 && nx < srcImg.width && ny >= 0 && ny < srcImg.height) {
        const nidx = ny * srcImg.width + nx;
        if (srcImg.data[nidx * 4 + 3] > 0 && dist[nidx] > curDist + 1) {
          dist[nidx] = curDist + 1;
...
for (let f = 0; f < 8; f++) {
  ...
  const w = maxX - minX + 1;
  const h = maxY - minY + 1;
  const subData = Buffer.alloc(w * h * 4); // RGBA
...
console.log('=== Validation of Generated GLM Target Strip ===');
for (let f = 0; f < 8; f++) {
...
fs.writeFileSync('C:/Users/15528/.dsh/plugins/dsh-reasoning-effort/assets/glm-runner-strip.png', encoded);
console.log('Successfully saved C:/Users/15528/.dsh/plugins/dsh-reasoning-effort/assets/glm-runner-strip.png (' + encoded.length + ' bytes)');
